...
5. Build - a Sonnet agent writes a static HTML prototype for each of the 3 winners
   into ./outputs.
...
## Budget
The smoke test on 10 interviews cost ~370k tokens. Extraction scales with the interview count (~13k each); ideation + the 3 HTML builds are roughly fixed (~230k). So the full 100 lands around 2M. **A 200k cap is far too low for 100.** Set a 2-3M cap, or process a subset (e.g., "the first 20 interviews") for a faster, cheaper demo. See `outputs/smoke-10.md` for the calibration run.
